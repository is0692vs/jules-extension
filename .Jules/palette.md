...
## 2026-08-16 - 無効化状態の理由の明示
**Learning:** 非同期処理中や状態によってインタラクティブな要素（ボタンなど）を無効にする際、単に disabled 属性に頼るだけではスクリーンリーダーなどの支援技術ユーザーにコンテキストが伝わらない。
**Action:** 無効化する際は常に title および aria-label 属性を設定し、なぜ無効化されているのか（例：'Cannot send without an active session'）をすべてのユーザーに明示的に説明する。
