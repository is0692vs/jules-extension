...
## 2026-08-25 - スクロール可能なコンテナのキーボードアクセシビリティ
**Learning:** 高さが制限されたスクロール領域（チャットビューやログの詳細など）は、キーボードのみのユーザーがアクセスできるように明示的なフォーカス管理（tabindex、regionロール）が必要です。
**Action:** `overflow: auto`を持つコンテナには、必ず `tabindex="0"`、`role="region"`、`aria-label`、および `focus-visible` スタイルを追加します。
