...
## 2026-08-22 - スクロール可能なコンテナのキーボードアクセシビリティ
**Learning:** `overflow: auto` や `overflow-y: auto` が設定され、高さが制限されたカスタムのスクロール可能なコンテナ（例：コードブロックや詳細コンテンツ領域）は、そのままではキーボード操作（Tabキーによるフォーカス）でスクロールできない場合があります。これにより、マウスを使えないユーザーがコンテンツ全体を閲覧できなくなるアクセシビリティ上の深刻な問題が発生します。
**Action:** カスタムのスクロール可能な要素には、必ず `tabindex="0"`、適切な `role="region"`、そして文脈に応じた `aria-label` を付与し、さらに `:focus-visible` 擬似クラスを用いてフォーカス時の視覚的フィードバック（アウトラインなど）を実装すること。
