...
## 2026-08-26 - カスタムスクロール領域のキーボードアクセシビリティ
**Learning:** `overflow: auto`（または `overflow-y: auto`）を用いて高さを制限したカスタムコンテナは、キーボードフォーカスを受け取らないため、マウスを使用しないユーザーが内容をスクロールできなくなるアクセシビリティの問題が生じます。
**Action:** スクロール可能な要素には常に `tabindex="0"` を付与し、`role="region"` と適切な `aria-label`、さらに `focus-visible` スタイルを追加することで、キーボード操作でフォーカスおよびスクロールが可能になるようにすること。
