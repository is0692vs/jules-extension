...
## 2026-08-05 - 送信ボタン無効化時のフォーカス復元（Composer）
**Learning:** フォーム送信時に送信ボタンを無効化すると、そのボタンにフォーカスがあった場合、フォーカスがドキュメント全体に移動し、キーボード操作が途切れてしまう。これは  のようなカスタムWebView内でも同様に発生する。
**Action:** 送信イベントハンドラ内で `document.activeElement === submitButton` をチェックし、該当する場合は `textarea.focus()` を呼び出してフォーカスをメインの入力フィールドに戻す。
## 2026-08-05 - 送信ボタン無効化時のフォーカス復元（Composer）
**Learning:** フォーム送信時に送信ボタンを無効化すると、そのボタンにフォーカスがあった場合、フォーカスがドキュメント全体に移動し、キーボード操作が途切れてしまう。これは composer.ts のようなカスタムWebView内でも同様に発生する。
**Action:** 送信イベントハンドラ内で document.activeElement === submitButton をチェックし、該当する場合は textarea.focus() を呼び出してフォーカスをメインの入力フィールドに戻す。
