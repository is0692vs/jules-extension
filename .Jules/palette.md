...
## 2026-08-21 - 無効化されたチェックボックスの理由の明示
**Learning:** 非同期処理中にフォームのチェックボックスなどを無効化する際、disabled 属性だけでなく、title と aria-label も設定することで、スクリーンリーダーやマウスユーザーに「なぜ操作できないのか」を明示的に伝えることが重要。
**Action:** 送信中の無効化処理などでは、各要素に対して、そのコンテキストに応じた (Cannot change while sending) などの具体的な説明を付与すること。
