...
## 2026-08-20 - [Performance] 巨大な配列でのスプレッド構文の使用回避
**Learning:** マークダウンドキュメントのパースなどで生成される巨大な動的配列に対して、`Math.max(...matches)` のようにスプレッド構文を使用すると、「Maximum call stack size exceeded」エラーが発生し、不必要なメモリ割り当てが生じます。
**Action:** サイズが不定、または非常に大きくなる可能性のある配列を処理する場合は、スプレッド構文を避け、`for...of` などの命令型ループを使用してください。
